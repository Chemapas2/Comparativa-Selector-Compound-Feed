...
def format_bulk_matrix_for_export(matrix_df: pd.DataFrame) -> pd.DataFrame:
    ...
    numeric_cols = [col for col in export_df.columns if any(token in col for token in ["score_", "precio_", "dif_precio_"]) or col in {"precio_origen_eur_t", "opcion_elegida"}]
    for col in numeric_cols:
